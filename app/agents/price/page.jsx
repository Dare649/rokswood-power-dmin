...
const Prices = ({ id }) => {
    ...
    const fetchPrice = async () => {
        ...
        try {
            ...
            setPrice(response.data.data || []); // Adjusted to match the new API structure
        } catch (error) {
            Swal.fire({
                icon: 'error',
                    ...
                    <tbody>
                        {price.length === 0 ? (
                            ...
                            price.map((item, index) => (
                                <tr key={index} className="border-t border-gray-200 capitalize cursor-pointer odd:bg-neutral1/50">
                                    ...
                                    <td className="px-5 py-2">{item.profit_per_unit}</td>
